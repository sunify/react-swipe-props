...
export default function App() {
  ...
  const updateRefs = (pos, transition) => {
    if (swipeRef.current) {
      swipeRef.current.style.transition = transition ? 'left 0.3s' : 'none';
      swipeRef.current.style.left = `${-pos * 100}%`;
    }
    if (progressRef.current) {
      progressRef.current.style.transition = transition ? 'width 0.3s' : 'none';
      progressRef.current.style.width = `calc(${100 - Math.min(100, pos / (count - 1) * 100)}% - 30px)`;
    }
    numRefs.forEach((numRef, i) => {
      if (numRef.current) {
        numRef.current.style.transition = transition ? 'transform 0.3s, opacity 0.3s' : 'none';
        numRef.current.style.transform = `scale(${((count - Math.abs(i - pos)) / count) ** 8 * 1.3})`;
      ...
      {(pos) => {
        return (
          <div className="swipeableRoot">
            ...
            <div className="swipeable" ref={swipeRef} style={{
              left: `${-pos * 100}%`
            }}>
              {range(0, count).map(i => {
                return <div key={i}><span ref={numRefs[i]} style={{
                  transform: `scale(${((count - Math.abs(i - pos)) / count) ** 8 * 1.3})`,
                  opacity: 1 - Math.abs(i - pos) / count ** 0.2
                }}>{i + 1}</span></div>;
              })}
